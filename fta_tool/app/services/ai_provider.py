    ...
    def _build_prompt(
        ...
        if analysis_context.get("system_context") or analysis_context.get("incident_context"):
            ...
            parts.append(
                ...
                "子要因は必ず親要因と同じ系統（DNS系・認証基盤系・ファイアウォール系・経路系など）の"
                "直接原因にすること。親が DNS 系なら DNS 系、認証基盤系なら認証基盤系、"
                "ファイアウォール系ならファイアウォール系の原因を深掘りする。\n"
                "コンテキストに複数の構成要素（DNS・認証基盤・ファイアウォール等）が登場しても、"
                "親要因と別系統の構成要素を、因果関係の根拠なく子要因に混ぜないこと。\n"
                "別系統の要因が重要だと考える場合でも、それは現在の親要因の子ではなく"
                "頂上事象の別の一次要因として扱うべきものなので、ここでは出力しないこと。\n"
